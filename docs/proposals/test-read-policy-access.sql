-- Transactional test: all changes rolled back. Proposal is NOT applied persistently.
begin;
insert into auth.users(id,email) values ('00000000-0000-4000-8000-000000009123','perf-role-test@example.invalid');
create temp table perf_access(phase text, actor text, tbl text, n bigint);
grant insert,select on perf_access to authenticated;
do $$
declare ro text; tbl text; n bigint;
begin
 foreach ro in array array['exec','admin','finance','sales','field_sales','stock','hr','lead_tech','tech','assistant','graphic','maid'] loop
  perform set_config('request.jwt.claims','{}',true);
  update profiles set role=ro,active=true where id='00000000-0000-4000-8000-000000009123';
  perform set_config('request.jwt.claims','{"sub":"00000000-0000-4000-8000-000000009123","role":"authenticated"}',true);
  execute 'set local role authenticated';
  foreach tbl in array array['quotations','quotation_items','boqs','boq_items','purchase_orders','po_items','hr_pay','hr_profiles','expense_requests','transactions','sub_payouts'] loop
   execute format('select count(*) from public.%I',tbl) into n;
   insert into perf_access values('before',ro,tbl,n);
  end loop;
  execute 'reset role';
 end loop;
end $$;
...
begin
 foreach ro in array array['exec','admin','finance','sales','field_sales','stock','hr','lead_tech','tech','assistant','graphic','maid'] loop
  ...
  foreach tbl in array array['quotations','quotation_items','boqs','boq_items','purchase_orders','po_items','hr_pay','hr_profiles','expense_requests','transactions','sub_payouts'] loop
   execute format('select count(*) from public.%I',tbl) into n;
   insert into perf_access values('after',ro,tbl,n);
  end loop;
  execute 'reset role';
 end loop;
end $$;
...
do $$ begin
 if exists(select 1 from perf_access b full join perf_access a on a.phase='after' and b.phase='before' and a.actor=b.actor and a.tbl=b.tbl where b.phase='before' and a.n is distinct from b.n) then raise exception 'Access changed'; end if;
end $$;
select count(*) filter(where phase='before') as comparisons, 'PASS 12 roles × 11 tables, counts unchanged' as result from perf_access;
rollback;
