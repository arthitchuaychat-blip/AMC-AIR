...
**หน้ารีวิวลูกค้า + ส่งขึ้นเว็บ (v591, 2026-08-06 — mig 208).** ข้อ 1 จากรีวิว แอป+เว็บ (6 ส.ค.): ปิดลูป คะแนน→การตลาด. module ใหม่ **`reviews`** (🌟 "รีวิวลูกค้า", กลุ่ม crm) — Reviews.jsx รวมงานที่ลูกค้าให้คะแนน (job_orders.cust_rating) + ปุ่ม **⬆️ ส่งขึ้นเว็บ / ลบจากเว็บ**. เว็บ amcair.net ดึงรีวิวจากตาราง **web_reviews (mig 134)** อยู่แล้ว (loadReviews, active/sort) → "ส่งขึ้นเว็บ" = insert web_reviews {name=คุณ+ชื่อต้น, role, text=comment, stars, source_job=job_no} · "ลบจากเว็บ" = delete แถวนั้น. **mig 208** เพิ่ม `web_reviews.source_job` (โยงกลับใบงาน · กันส่งซ้ำ + รู้ว่ารีวิวไหน published). api: `listReviews()` (rated jobs + published state จาก web_reviews.source_job) · `publishReview({job_no,name,role,text,stars})` (fallback ตัด source_job ถ้ายังไม่รัน 208) · `unpublishReview(webReviewId)`. permissions module `reviews` E: admin/exec/sales/graphic · V: hr. รีวิวที่ทีมกราฟิกพิมพ์เอง (source_job=null) ยังจัดการที่ WebManage → รีวิวลูกค้า เหมือนเดิม. **ต่อยอด:** ★ badge บนใบงาน (ยังไม่ทำ) · เตือนขอคะแนนอัตโนมัติหลังปิดงาน.
