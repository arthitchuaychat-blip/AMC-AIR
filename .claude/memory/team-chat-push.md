...
**ตอบกลับ + รีแอกชัน + ลบข้อความตัวเอง (v553, 2026-08-02, mig 190).** chat_messages + `reply_to bigint`, `deleted_at` · ตาราง `chat_reactions(message_id,user_id,emoji)` PK รวม + RLS (อ่าน=สมาชิกห้อง, เขียน/ลบ=ของตัวเอง) + realtime · RPC `chat_delete_message` (soft, sender only, ล้างเนื้อหา). api: listChatMessages แนบ `m.reactions={emoji:{count,mine}}` · sendChatMessage(roomId,text,mentionIds,**replyTo**) + fallback · `toggleReaction(msgId,emoji,on)` · `deleteChatMessage`. TeamChat.jsx: state replyTo/rxFor, ปุ่ม ↩/😊/🗑 ต่อข้อความ (tc-msg-acts), reply preview (tc-reply-quote), reactions row (tc-rx), reply bar เหนือ compose, realtime ฟัง chat_reactions → loadMsgs. ข้อความที่ลบโชว์ "🚫 ข้อความถูกลบ". **ChatDock (mini) ยังไม่รองรับ reaction/reply/deleted** (โชว์ข้อความว่างถ้าลบ) — full page เท่านั้น. **ค้างทำ (เจ้าของขอ):** กลุ่ม B (ปิดเสียงรายห้อง + ค้นในห้อง + refactor countUnread N+1) · กลุ่ม C (เปิดแชตห้องงานจากใบงาน + สร้างงาน/ทาสก์จากข้อความ).
