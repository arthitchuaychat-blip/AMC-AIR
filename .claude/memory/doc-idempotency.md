...
**ชุดหลายแถว (v848):** `transactions` (เบิก/คืน/ชำรุด/รับของ) ใช้ unique ต่อแถวไม่ได้ → **ตารางจอง `write_requests`** (PK request_id, mig `20260917130000`): `recordTransactions(rows, { request_id })` จองก่อน insert ทั้งชุด · ซ้ำ = โยน error `e.dup=true` (หน้าจอบอก "บันทึกไปแล้ว" + รีเซ็ต ไม่คิดต้นทุน/เบิกคู่แฝดซ้ำ) · insert ล้ม = `_releaseRequest` · ตารางไม่มี = ข้ามการจอง. หน้าจอ: `useRef` ต่อ flow (`reqRef1/2`) สร้าง UUID เมื่อยังไม่มี **ล้างเมื่อสำเร็จเท่านั้น** (retry ใช้ตัวเดิม) · ชุดคู่แฝดใช้ `id + ":job"`. รับของ PO เต็มใบมี optimistic lock (`markPoReceived .eq status open`) อยู่แล้ว · applyStockCount มี revert. test `test-txn-idempotency.mjs`.

**migration รันแล้วทั้งหมด (ยืนยันผ่าน REST 2026-09-17):** doc_request_id (v844) · expense_request_id (v847) · write_requests (v848) — ไม่ต้องขอให้เจ้าของรันซ้ำ
