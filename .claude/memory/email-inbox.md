...
**ส่งเอกสารขายทางอีเมลจากหน้าแชต (v659, ส.ค. 2026):** ในแชตลูกค้า → ประวัติเอกสาร → ปุ่ม 📤 ส่ง → popup มีทั้ง "ส่งเข้าแชต (รูป/PDF)" และ **"📧 ส่งอีเมล"** (โชว์เมื่อลูกค้าที่ผูกไว้มีอีเมล). กด → `startSend(e,"email")` → DocCapture render เอกสาร → `captureDocForEmail(node,"pdf")` แนบ PDF จริง → `EmailDocModal` (แก้ to/หัวข้อ/ข้อความ) → `sendEmail({to,subject,text,attachments})`. รองรับ quote/invoice/**billing**/receipt/creditnote/debitnote — **ใบวางบิลเพิ่ง add**: billingSlip ใน DocCapture + billing เข้า `listCustomerDocs` (destructure `bn`) + docmeta TYPE_LABEL/DOC_FILTERS/DOC_STATUS. **คอนเฟิมงาน**: ปุ่ม 📧 ในตัวเลือกใบงาน (jobPicker) → อีเมล buildOrderConfirm เป็น body (ไม่มีไฟล์แนบ · EmailDocModal ยอมส่งถ้ามี body หรือ atts). ใช้ได้ทั้ง LINE/FB (อีเมลไม่ขึ้นกับช่องแชต). email-send รับ attachments `[{name,url,mimeType}]` ดึง URL เองแล้ว MIME multipart. → **แนบไฟล์ในเมลทำได้แล้ว** (ผ่าน endpoint เดิม).
**ยังไม่ทำ/ต่อยอด:** ผูกลูกค้าในกล่องเมล (linkEmailCustomer มีแล้วแต่ยังไม่มี UI picker) · เตือนเมลใหม่ (ตอนนี้ต้องเปิดหน้า/รีเฟรช) · DKIM กันตกขยะ (ตั้งใน Admin ทีหลัง). Vercel Hobby cron เต็ม 2 ตัวแล้ว — ถ้าจะ auto-sync ต้องรวม daily-cron หรือใช้ pull-on-open ต่อไป.
