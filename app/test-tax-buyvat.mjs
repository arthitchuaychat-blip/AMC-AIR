...
// ความจริงปัจจุบัน (หลังเพิ่ม VAT จากใบเบิกจ่าย): ภาษีซื้อ = PO ที่รับของ/จ่ายแล้ว + บิลหน้างานที่ติ๊ก VAT ในเมนูเบิกจ่าย
// เจตนาเดิมของเทสต์ยังอยู่: "จอต้องบอกตรงกับที่นับจริง" — แค่ที่นับจริงเปลี่ยนแล้ว ป้าย/หมายเหตุจึงต้องพูดตามนั้น
check("ป้ายบนการ์ดบอกว่านับทั้ง PO และบิลหน้างาน", /ภาษีซื้อ \(PO \+ บิลหน้างาน/.test(s));
check("หมายเหตุท้ายหน้าบอกว่า PO นับเฉพาะใบที่รับของ/จ่ายแล้ว", /เฉพาะใบที่รับของ\/จ่ายแล้ว/.test(s),
  "ถ้าไม่บอก คนอ่านจะคิดว่า PO ที่สั่งค้างไว้ถูกนับด้วย");
check("หมายเหตุท้ายหน้าบอกว่ารวมบิลหน้างาน (ใบเบิกจ่ายที่ติ๊ก VAT) แล้ว", /\+ บิลหน้างาน/.test(s),
  "ถ้าไม่บอก คนอ่านจะคิดว่าตัวเลขนี้ยังขาดบิลหน้างาน");
