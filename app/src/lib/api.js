...
export async function payFinancingInstallment(id) {
  ...
  requireFinancingState(ln);   // fail-closed: ไม่มีสถานะงวด → หยุด ไม่สร้างใบเบิก
  ...
  // ตั้งเบิกไม่ใช่จ่ายเงินจริง — ห้าม fallback ไปเพิ่ม paid_count (จะทำให้ "ตั้งจ่าย" ถูกนับเป็น "จ่ายแล้ว")
...
export async function autoDebitFinancing(id) {
  ...
  const { data: ln, error } = await supabase.from("loans").select("*").eq("id", id).maybeSingle();
  if (error || !ln) throw error || new Error("ไม่พบสัญญา");
  requireFinancingState(ln);   // fail-closed
  const seq = (Number(ln.paid_count) || 0) + 1;
  if (seq > (Number(ln.term_months) || 0)) throw new Error("ผ่อนครบทุกงวดแล้ว");
...
export async function confirmFinancingPaid(id) {
  ...
  const target = requireSubmittedInstallment(ln);   // เฉพาะงวดถัดไปที่ตั้งเบิกไว้จริง — ไม่เดินงวดเอง
