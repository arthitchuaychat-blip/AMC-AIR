...
const METHODS = ["เงินสด", "โอนเงิน", "เช็ค", "บัตรเครดิต", "Trade Account"];
...
export default function Receipts({ role, fromInvoice, onFromInvoiceConsumed, onOpenDoc, focus, onFocusConsumed, onGoChat }) {
  ...
  async function del(x) { const reason = await confirmDialog({ title: `ลบใบเสร็จ ${x.receipt_no}?`, message: "ใบแจ้งหนี้จะกลับเป็นค้างชำระ · ข้อมูลจะถูกเก็บไว้ในประวัติการลบ (กู้คืนได้)", confirmText: "ลบ", prompt: { label: "เหตุผลที่ลบ (ไม่บังคับ)", placeholder: "เช่น ออกผิด · รับเงินผิดยอด" } }); if (reason === false) return; try { await deleteReceipt(x.receipt_no, x.invoice_no, reason); flash("ลบแล้ว"); await load(); } catch (e) { flash("ลบไม่สำเร็จ: " + (e.message || e), true); } }
  async function cancel(x) { const reason = await confirmDialog({ title: `ยกเลิกใบเสร็จ ${x.receipt_no}?`, message: "เก็บประวัติไว้ · ใบแจ้งหนี้กลับเป็นค้างชำระ", confirmText: "ยกเลิกใบนี้", prompt: { label: "เหตุผลที่ยกเลิก (ไม่บังคับ)", placeholder: "เช่น ยกเลิกรายการ" } }); if (reason === false) return; try { await setReceiptStatus(x.receipt_no, "cancelled", x.invoice_no, reason); flash("ยกเลิกแล้ว"); await load(); } catch (e) { flash("ไม่สำเร็จ: " + (e.message || e), true); } }
