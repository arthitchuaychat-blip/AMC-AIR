...
function PayRow({ p, onSave }) {
  ...
  const saveBase = (val) => {
    ...
    // เรต OT อิงฐานเสมอ (อัปเดตทุกครั้งที่เปลี่ยนฐาน) — รายเดือน ÷30÷8×1.5 · รายวัน ÷8×1.5
    if (v > 0) {
      const auto = isMonthly ? calcAutoOt(v) : autoDailyOt(v);
      if (auto !== (Number(p.ot_rate) || 0)) { fields.ot_rate = auto; setOtRate(auto); }
    }
  ...
  return (
    <div className="hr-pay-row">
      ...
      <span className="inp inp-unit" style={{ width: 210, opacity: .8 }} title={payType === "daily" ? "เรต OT รายวันคิดจากค่าแรง/วันอัตโนมัติ: ค่าแรงวัน ÷ 8 × 1.5" : "เรต OT รายเดือนคิดจากเงินเดือนอัตโนมัติ: เงินเดือน ÷ 30 ÷ 8 × 1.5"}><span className="unit-pre">OT ฿</span>
        <input type="number" value={payType === "daily" ? autoDailyOt(basePay) : calcAutoOt(basePay)} readOnly tabIndex={-1} style={{ cursor: "default" }} /><span className="unit-suf">/ชม. · อัตโนมัติ</span></span>
