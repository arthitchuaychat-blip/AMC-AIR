            ...
            <div className={"card job-card doc2" + (po.status !== "open" ? " closed" : "")} key={po.po_no}>
              <DocCardHead no={po.po_no} onClick={() => openPeek("po", po.po_no)} partyIcon="🏭"
                badges={<>
                  ...
                  {po.expense_id && <button type="button" className="vat-badge" style={{ cursor: "pointer", border: "1px solid transparent", background: "#ecfeff", color: "#0e7490" }} title="ไปที่เมนูเบิกจ่าย (ใบเบิกที่ผูกกับใบสั่งซื้อนี้)" onClick={(e) => { e.stopPropagation(); onGoExpenses && onGoExpenses(); }}>💳 เบิก #{String(po.expense_id).slice(0, 8).toUpperCase()} ↗</button>}
                </>}
