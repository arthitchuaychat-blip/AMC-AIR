...
import { fmtBaht, fmtNum, custCode, fmtDocDate, fmtDocAmount } from "../lib/format";
...
function noteSlip(x, companies) {
  ...
  const its = x.items || [];
  return (
    <DocSlip currencyUnit="บาท" company={co} titleTh={isCredit ? "ใบลดหนี้" : "ใบเพิ่มหนี้"} titleEn={isCredit ? "CREDIT NOTE" : "DEBIT NOTE"} docNo={x.note_no}
      ...
      totals={<div className="doc-totals">
        <div><span>รวมยอด{verb}ก่อนภาษี</span><b>{fmtDocAmount(x.base)}</b></div>
        ...
        {x.wht_amt > 0 && <div><span>หัก ณ ที่จ่าย {Number(x.wht_rate) || 3}%</span><b>− {fmtDocAmount(x.wht_amt)}</b></div>}
        <div className="doc-grand"><span>ยอดสุทธิ ({verb})</span><b>{fmtDocAmount(x.net)}</b></div>
      </div>}>
      {its.map((it, i) => (
  ...
  return (
    <DocSlip currencyUnit="บาท" company={co} titleTh="ใบเสนอราคา" titleEn="QUOTATION" docNo={q.quote_no}
      ...
      totals={<div className="doc-totals">
        ...
        {q.whtOn ? <div><span>หัก ณ ที่จ่าย {Number(q.wht_rate ?? 3)}%</span><b>− {fmtDocAmount(q.whtAmt)}</b></div> : null}
...
function invoiceSlip(x, q, companies) {
  ...
  return (
    <DocSlip currencyUnit="บาท" company={co} titleTh="ใบแจ้งหนี้" titleEn="INVOICE" docNo={x.invoice_no}
      ...
      totals={<div className="doc-totals">
        ...
        {x.wht_amt > 0 && <div><span>หัก ณ ที่จ่าย (ตอนชำระ)</span><b>− {fmtDocAmount(x.wht_amt)}</b></div>}
      ...
      {(() => { const hasD = (q?.items || []).some((x2) => Number(x2.discount) > 0); return (q?.items || []).map((it, i) => (
        <tr key={i}><td>{i + 1}</td><td>{it.item_code || "-"}</td><td>{it.name}{it.description ? <div className="doc-item-desc">{it.description}</div> : null}</td><td className="r">{Number(it.qty)} {it.unit || ""}</td><td className="r">{fmtDocAmount(it.price_show ?? it.unit_price)}</td>{hasD && <td className="r">{Number(it.discount) > 0 ? "− " + fmtDocAmount(it.discount) : "-"}</td>}<td className="r">{fmtDocAmount(Number(it.qty) * Number(it.price_show ?? it.unit_price) - (Number(it.discount) || 0))}</td></tr>
      )); })()}
    </DocSlip>
  );
}
...
function receiptSlip(x, q, inv, companies) {
  ...
  return (
    <DocSlip currencyUnit="บาท" company={co} titleTh={isVat ? "ใบเสร็จรับเงิน/ใบกำกับภาษี" : "ใบเสร็จรับเงิน"} titleEn={isVat ? "RECEIPT / TAX INVOICE" : "RECEIPT"} docNo={x.receipt_no}
