...
// สิทธิ์: เฉพาะทีมหลังบ้าน (ตรวจ JWT ผู้ใช้)
...
export default async function handler(req, res) {
  try {
    ...
    if (!ids.length) return res.status(200).json({ ok: true, new: 0, threads: 0 });
    ...
    for (const id of newIds) {
      try { parsed.push(parseMessage(await gmail(`messages/${id}?format=full`, token), self)); } catch { /* ข้ามฉบับที่พัง */ }
    }
    ...
    if (parsed.length) {
      ...
      await fetch(`${SB()}/rest/v1/email_messages`, { method: "POST", headers: { ...sbH(), Prefer: "resolution=merge-duplicates" }, body: JSON.stringify(rows) });
    }
    ...
    for (const tid of affected) {
      ...
      await fetch(`${SB()}/rest/v1/email_threads`, { method: "POST", headers: { ...sbH(), Prefer: "resolution=merge-duplicates" }, body: JSON.stringify(row) });
    }

    return res.status(200).json({ ok: true, new: parsed.length, threads: affected.length });
  } catch (e) {
