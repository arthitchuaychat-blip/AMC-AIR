...
begin
 r:=quotation_page('unique-pagination-fixture','2026-01-31','2026-01-31');
 if (r->>'total')::int<>123 or jsonb_array_length(r->'nos')<>50 then raise exception 'count/date range'; end if;
 a:=r->'nos'; b:=quotation_page('unique-pagination-fixture','2026-01-31','2026-01-31','all','all','','{}',50)->'nos';
 c:=quotation_page('unique-pagination-fixture','2026-01-31','2026-01-31','all','all','','{}',100)->'nos';
 if (select count(distinct x) from jsonb_array_elements_text(a||b||c) x)<>123 then raise exception 'duplicate/missing page'; end if;
 if (quotation_page('PERF-TEST-OLD')->>'total')::int<>1 then raise exception 'old quote search'; end if;
 if (quotation_page('unique-pagination-fixture',null,null,'approved')->>'total')::int<>0 then raise exception 'status filter'; end if;
 if (quotation_page('unique-pagination-fixture',null,null,'all','all','','{}',999)->'nos')<>'[]'::jsonb then raise exception 'empty page'; end if;
end $$;
