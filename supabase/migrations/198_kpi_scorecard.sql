-- 198_kpi_scorecard.sql — สกอร์การ์ดผลงาน (วัด KPI จริงจากข้อมูลที่มีอยู่แล้ว)
--
-- เปลี่ยน KPI ในคู่มือ (เขียนไว้เฉย ๆ) → "วัดจริงอัตโนมัติ" ต่อคน/ต่อทีม ในช่วงเวลาที่เลือก
-- RPC security definer + รวมข้อมูลฝั่งเซิร์ฟเวอร์ (กันเพดาน 1000 แถว) · จำกัดสิทธิ์เฉพาะออฟฟิศ
--
-- ยอดขายผูกกับ "คน" ผ่าน created_by บนใบเสนอราคา/ใบเสร็จ (คนที่สร้างเอกสาร = เจ้าของยอด)
-- งานผูกกับ "ทีม" ผ่าน assigned_team บนใบงาน
--
...
-- ── ฝ่ายขาย: ใบเสนอราคาที่ออก + ที่ปิดได้ (approved) ต่อคน ──
q as (
...
-- ── ฝ่ายขาย: ยอดเก็บเงินจริง (ใบเสร็จ net) ต่อคน ──
r as (
...
-- ── ทีมช่าง: งานเสร็จ / เคลม / คะแนนเฉลี่ย ต่อทีม ──
jt as (
...
-- ✅ ตรวจผล (รันในเอดิเตอร์ = service role, my_role() ว่าง → คืน sales/teams ว่าง ถือว่าปกติ;
--    ต้องเรียกผ่านแอปด้วยบัญชี admin/exec/hr/finance ถึงเห็นข้อมูลจริง)
select 'kpi_scorecard ready' as status;
