-- Head technicians supervise all teams, including accounts without an assigned team.
-- Only the team-membership check changes. Field transitions, closed/locked guards,
-- office approval, read policies, existing routine signatures and grants stay in place.
...
begin
 if public.app_actor_role() is null or public.app_actor_role()='hr' then raise exception 'ไม่มีสิทธิ์เข้าถึงงาน' using errcode='42501'; end if;
  ...
  if my_role() in ('admin','sales','exec','finance','stock') then
    null;  -- คงสิทธิ์เปลี่ยนสถานะของออฟฟิศตามเดิม
    ...
    if my_role() = 'tech' and (v_team = my_team()
...
begin
 if public.app_actor_role() is null or public.app_actor_role()='hr' then raise exception 'ไม่มีสิทธิ์เข้าถึงงาน' using errcode='42501'; end if;
  ...
  if (my_role() in ('admin','sales','exec','finance','stock')
          or my_role() = 'lead_tech'
          or (my_role() = 'tech' and v_team = my_team())) is not true then
    raise exception 'not allowed';
  end if;
